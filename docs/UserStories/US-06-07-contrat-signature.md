...
- Révision **`010_pg_dossier_status`** : sous **PostgreSQL** uniquement, ajoute les valeurs `en_signature` et `attente_livraison` au type ENUM `dossierstatusenum`. Sans elle, les mises à jour de statut lèvent `invalid input value for enum dossierstatusenum` (SQLite / tests inchangés : pas d’ENUM natif).

Après déploiement : `alembic upgrade head`.

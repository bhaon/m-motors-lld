...
Les overlays **dev**, **staging** et **production** envoient l’OTLP vers le **collecteur** `http://otel-collector.monitoring.svc.cluster.local:4318`, qui relaie vers **Jaeger**. Pour la topologie entre services, voir **Jaeger** (onglet dépendances) — **`kiali-topologie-services.md`** (Kiali non recommandé sans Istio).
